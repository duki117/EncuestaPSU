...
$idpsu = $_SESSION["id"];
//CONTANDORES PARA LAS TABLAS MALESTAR PSICOLOGICO Y RIESGO SUICIDA
$malestar=0;
$riesgo=0;
    ...
    if(!empty($_POST['sp-mp1']) >=1 &&
        ...
        !empty($_POST['sp-rs20']) >=1){
            ...
            for ($i = 1; $i <= 10; $i++) {
                if(${"resp" . $i}=='Nunca'){
                    $malestar+=1;
                }else if(${"resp" . $i}=='Pocas veces'){
                    $malestar+=2;
                }else if(${"resp" . $i}=='A veces'){
                    $malestar+=3;
                }else if(${"resp" . $i}=='Muchas veces'){
                    $malestar+=4;
                }else if(${"resp" . $i}=='Siempre'){
                    $malestar+=5;
                }
            }
            if($malestar>=10 && $malestar<=15){
                $psico="INSERT INTO malestarpsicologico(Id_psu, Resultado) VALUES ('$idpsu', 'RIESGO BAJO')";
            }else if($malestar>=16 && $malestar<=21){
                $psico="INSERT INTO malestarpsicologico(Id_psu, Resultado) VALUES ('$idpsu', 'RIESGO MODERADO')";
            }else if($malestar>=22 && $malestar<=29){
                $psico="INSERT INTO malestarpsicologico(Id_psu, Resultado) VALUES ('$idpsu', 'RIESGO ALTO')";
            }else if($malestar>=30 && $malestar<=50){
                $psico="INSERT INTO malestarpsicologico(Id_psu, Resultado) VALUES ('$idpsu', 'RIESGO MUY ALTO')";
            }
            ...
            $malpsi=mysqli_query($conex,$psico);
            ...
            $rest31 && $rest32 && $rest33 && $rest34 && $rest35 && $rest36 && $rest37 && $rest38 && $rest39 && $rest40 && $malpsi){
                $_SESSION["sp"]=TRUE;
                ?>
                <script>
                    location.reload();
